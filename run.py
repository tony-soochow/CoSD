...
def run(frames, eps_fixed=False, eps_frames=1e6, min_eps=0.01, eval_every=1000, eval_runs=20, worker=1):
    """Deep Q-Learning.
    ...
    log_dir = "logs-CoSD/" + args.env + "-" + time + ".txt"
    f = open(log_dir, "w")
    for frame in tqdm(range(1, frames + 1)):
        ...
        if done.any():
            ...
            print('\rEpisode {}\tFrame {} \tAverage Score: {:.2f}'.format(i_episode * worker, frame * worker,
                                                                          np.mean(scores_window)), end="", file=f,
                  flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-agent", type=str, choices=["dqn",
                                                     ...
                                                     "rainbow"], default="noisy_dueling",
                        help="Specify which type of DQN agent you want to train, default is DQN - baseline!")

    parser.add_argument("-env", type=str, default="Walker2dJump-v1", help="Name of meta env")
    parser.add_argument("--env_name", type=str, default="Walker2d-v3",help="Name of skill_env")
    parser.add_argument("-frames", type=int, default=int(2e6), help="Number of frames to train, default = 5 mio")
    parser.add_argument("-seed", type=int, default=123, help="Random seed to replicate training runs, default = 1")
    ...
    parser.add_argument("--fill_buffer", type=int, default=100000, help="Adding samples to the replay buffer based on a random policy, before agent-env-interaction. Input numer of preadded frames to the buffer, default = 50000")
    ...
    parser.add_argument("-n_skills", "--n_skills", type=int, default=10, help="Number of skills")
    parser.add_argument("-time_steps_per_skill", "--time_steps_per_skill", type=int, default=5,help="Number of skills steps")
    ...
    state_size = np.arange(0, n_states).shape
    print(state_size)
