...
class SACAgent:
    def __init__(self,
        ...
        self.env_prefix = re.split("-|_", self.config["env_name"])[0]
    ...
    def store(self, state, z, done, action, next_state, t_state,t_next_state):
        ...
        t_state = from_numpy(t_state).float().to("cpu")
        t_next_state = from_numpy(t_next_state).float().to("cpu")

        self.memory.add(state, z, done, action, next_state, t_state, t_next_state)
    ...
    def unpack(self, batch):
        ...
        t_obs_batch = torch.cat(batch.t_state).view(self.batch_size, 2).to(self.device)
        next_t_obs_batch = torch.cat(batch.t_next_state).view(self.batch_size, 2).to(self.device)

        return states, zs, dones, actions, next_states, t_obs_batch, next_t_obs_batch

    def transform_state(self, state):
        if self.obs_transform:
            return self.obs_transform(self.env, state)
        return state


    def train(self, z_pro_memory):        
        if len(self.memory) < self.batch_size:
            return None
        else:
            batch = self.memory.sample(self.batch_size)          
            states, zs, dones, actions, next_states, t_obs_batch, next_t_obs_batch = self.unpack(batch) #find sigema s' batch
            ...
            #按batch_size中z的顺序计算 H(S|Z)
            for i in range(self.batch_size):  
                z = zs[i].item()
                penalty = z_pro_memory[z].get_penalty( t_obs_batch[i], next_t_obs_batch[i])     
                penalties.append(penalty)
                            
            consistency_penalty = torch.cat(penalties, dim=0)
            ...
            rewards = logq_z_ns.gather(-1, zs).detach() - torch.log(p_z + 1e-6) + consistency_penalty
