...
class Logger:
    def __init__(self, agent, **config):
        self.config = config
        self.agent = agent
        self.log_dir = self.config["env_name"][:-3] + "/" + "n_skills" + "_" + str(self.config["n_skills"])
        self.start_time = 0
        self.duration = 0
    ...
    def load_weights(self):
        model_dir = glob.glob("Checkpoints/" + self.config["env_name"][:-3] + "/" + "n_skills" + "_" + str(self.config["n_skills"]) + "/")
        model_dir.sort()
